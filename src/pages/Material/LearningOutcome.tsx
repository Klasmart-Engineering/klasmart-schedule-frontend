...
interface filterGroupProps {
  ...
  filterGroupData: LearningComesFilterQuery;
...
function SelectGroup(props: filterGroupProps) {
  ...
  const autocompleteChange = async (value: any | null, name: "subjects" | "category" | "subs" | "ages" | "grades" | "programs") => {
    ...
    const filterData =
      name === "programs" && !value.length
        ? { programs: [], subjects: [], category: [], subs: [], ages: [], grades: [] }
        : {
            ...filterQuery,
    ...
    const filterQueryAssembly = {
      ...
      category_ids: values(filterResult.category),
  ...
  const defaultValues = (enumType: "subjects" | "category" | "subs" | "ages" | "grades") =>
  ...
  const selectGroup = [
    ...
    { name: d("Category").t("library_label_category"), data: deduplication(filteredList.category), enum: "category" },
  ...
  const autocompleteValue = (key: string) => {
    return key === "programs"
      ...
      : defaultValues(key as "subjects" | "category" | "subs" | "ages" | "grades");
  };
          ...
          {selectGroup.map((item, index) => (
            <Autocomplete
              ...
              onChange={(e: any, newValue) => {
                autocompleteChange(newValue, item.enum as "subjects" | "category" | "subs" | "ages" | "grades");
              }}
  ...
  return (
    <Box className={classes.previewContainerMb} style={{ height: `${boxHeight}px` }}>
      ...
      <div className={classes.resultText}>
        <span>
          {learningOutComeTotal} {d("Results").t("schedule_lesson_plan_popup_results")}
        </span>
...
export default function LearningOutcome(props: InfoProps) {
  const classes = useStyles();
  const {
    programs,
  ...
  const [filterQuery, setFilterQuery] = React.useState<LearningComesFilterQuery>(filterGroupData);

  const getLearningFiled = (ids: string[]) => {
    const categoryAssembly = modelSchedule.getLearningOutcomeCategory(skills.concat(developmental), ids ?? []);
  ...
  const [checkAssumed, setCheckAssumed] = React.useState<boolean>(learningOutcomeData.is_assumed);

  const [searchValuePc, setSearchValuePc] = React.useState<string>(learningOutcomeData.search_value);
  ...
  const reset = () => {
    ...
    searchOutcomesList(getFilterQueryAssembly({ programs: [], subjects: [], category: [], subs: [], ages: [], grades: [] }));
    setSearchValuePc("");
  };
  ...
  const saveDisabled = useMemo(() => {
    return !selectIds.length || scheduleDetail.complete_assessment;
  }, [selectIds, scheduleDetail]);
  ...
  return mobile ? (
    <ScheduleLessonPlanMb
      ...
      searchName={learningOutcomeData.search_value}
      learningOutComeTotal={content_lists.length}
      reset={reset}
