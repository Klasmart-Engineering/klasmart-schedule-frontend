        ...
        label={
          <div className={classes.labelRoot}>
            ...
            >
              <Typography
                ...
                className={clsx(
                  ...
                  isOnlyMine ? classes.abbreviationLabel : ""
                )}
              >
                {labelText}
              </Typography>
            ...
            {filterItem.label === "classType" && (
              <div className={classes.classTypeColor} style={{ backgroundColor: getClassTypeColor(self_id) }} />
            )}
          </div>
        }
        classes={{
...
function FilterTemplate(props: FilterProps) {
  ...
  const handleChangeExits = async (data: string[], checked: boolean, node: FilterItemInfo, existData: string[]) => {
    ...
    if (node.label === "program" && !checked) {
      ...
      const deepSubject = stateSubject.filter((v: InterfaceSubject) => {
  ...
  const openClassMenuByOther = async () => {
    setShowUserMenu(false);
    await getUserOfUndefined("", true, ConnectionDirection.Forward);
    setUserPageYFun();
  ...
  const styledTreeItemTemplate = (treeItem: FilterDataItemsProps[]) => {
    ...
    return treeItem.map((item: FilterDataItemsProps) => {
      return (
        !item.isHide && (
          <div key={item.id}>
            {["School+1", "Programs+1"].includes(item.id) && <div className={css.filterRouter} />}
            <StyledTreeItem
              nodeId={item.id}
              labelText={item.name}
              handleChangeShowAnyTime={handleChangeShowAnyTime}
