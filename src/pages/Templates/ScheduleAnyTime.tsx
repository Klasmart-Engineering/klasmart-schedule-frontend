...
function AnyTimeSchedule(props: SearchListProps) {
  ...
  const handleGoLive = async (scheduleDetail: EntityScheduleListView) => {
    ...
    //   getScheduleLiveToken({ schedule_id: scheduleDetail.id as string, live_token_type: "live", metaLoading: true })
    // );
    // if (resultInfo.payload.token) {
    if (privilegedMembers("Student") && scheduleDetail.class_type_label?.id === "Homework") {
      toLive(scheduleDetail.id);
      // toLive(scheduleDetail.id, resultInfo.payload.token);
      return;
    } else {
      toLive(scheduleDetail.id);
    }
  ...
  const buttonGroup = (type: string, scheduleInfo: EntityScheduleListView, showDeleteButton: boolean) => {
  ...
  const buttonGroupMb = (scheduleInfo: EntityScheduleListView, showDeleteButton: boolean) => {
    const isScheduleExpiredMulti = (): boolean => {
      if (scheduleInfo.is_home_fun) {
        return scheduleInfo.id ? scheduleInfo.status !== "NotStart" || privilegedMembers("Student") : false;
      } else {
    ...
    return (
      <span>
        {showDeleteButton && !(!scheduleInfo.is_home_fun && scheduleInfo.assessment_status === "complete") && (
          <EditOutlined
...
interface InfoMbProps extends SearchListProps {
  handleGoLive: (scheduleDetail: EntityScheduleListView) => void;
  anyTimeData: AnyTimeData;
  buttonGroupMb: (scheduleInfo: EntityScheduleListView, showDeleteButton: boolean) => JSX.Element;
}
