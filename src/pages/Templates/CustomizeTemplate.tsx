...
interface InfoMbProps extends InfoProps {
  ...
  timestampToTime: (timestamp: number, is_year: boolean, is_month?: boolean) => string;
  multiStructure: (item?: EntityScheduleShortInfo[]) => string[] | undefined;
  handleGoLive: (scheduleInfos: ScheduleEditExtend) => void;
  checkFileExist: (source_id?: string) => Promise<boolean | undefined>;
}

function CustomizeTemplateMb(props: InfoMbProps) {
...
export default function CustomizeTemplate(props: InfoProps) {
  const classes = useStyles();
  ...
  const timestampToTime = (timestamp: number, is_year: boolean, is_month?: boolean): string => {
    ...
    if (is_year) return `${weekArr[W]}, ${monthArr[M]} ${D}, ${Y}`;
