...
export function ConfirmDialog() {
  const { open, title, content, type, label, confirmText, cancelText } = useSelector<RootState, RootState['confirm']>(state => state.confirm)
    ...
    <Dialog open={open} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" disableBackdropClick>
      ...
      <DialogContent>
        ...
        {type === ConfirmDialogType.textField && (
          <Controller name={INPUT_NAME} as={TextField} variant="standard" autoFocus={true} fullWidth label={label} rules={{ required: true }} control={control} />
        )}
      </DialogContent>
      <DialogActions>
        <Button onClick={handleCancel} color="primary" autoFocus>{cancelText}</Button>
